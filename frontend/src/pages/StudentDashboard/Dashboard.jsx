...
import { Home, ClipboardList, Send, HelpCircle, ChevronDown } from "lucide-react";
...
const StudentDashboard = () => {
  ...
  const [selectedForm, setSelectedForm] = useState("Proforma Form");
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-green-50 to-white text-gray-800 font-sans">
      {/* Sidebar */}
      <aside className="w-72 bg-gradient-to-b from-green-50 to-white shadow-xl flex flex-col justify-between p-6 rounded-r-3xl">
        <div>
          {/* Logo Section */}
          <div className="flex flex-col items-center mb-8">
            <img
              src="/logo.png" // <-- apna logo path rakho public folder me (frontend/public/logo.png)
              alt="UOK Logo"
          ...
          <nav className="flex flex-col gap-4 text-sm">
            ...
            <button
              ...
              className="flex items-center gap-3 px-5 py-4 rounded-2xl bg-white shadow hover:bg-green-50 hover:shadow-lg text-gray-700 font-medium transition-all"
            >
      ...
      <main className="flex-1">
        {/* Header */}
        <Header studentName={studentName} adminName={adminName} />
            ...
            {hasSubmitted ? (
              <div className="flex gap-6 overflow-x-auto pb-4">
                {statuses.map((item, idx) => (
                  <div
                    key={idx}
                    className={`flex-shrink-0 w-70 h-64 relative group p-6 rounded-2xl border border-green-100 shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 
